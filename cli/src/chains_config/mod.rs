...
const EXCLUDE_CHAINS: [&str; 8] = ["Polkadot", "Kusama", "Westend", "Moonbeam", "Moonriver", "Moonbase Alpha", "Moonbase Relay Testnet", "Arctic Relay Testnet"];
...
pub(crate) fn update_chains_config(chains_opts: ChainsOpts) -> Result<()> {
  ...
  let chain_params = match chains_opts.env.as_str() {
    "dev" => ("config_dev.toml", "public/data_dev.json", format!("https://raw.githubusercontent.com/nova-wallet/nova-utils/master/chains/{}/{}", chains_opts.version, "chains_dev.json")),
    "prod" => ("config.toml", "public/data.json", format!("https://raw.githubusercontent.com/nova-wallet/nova-utils/master/chains/{}/{}", chains_opts.version, "chains.json")),
